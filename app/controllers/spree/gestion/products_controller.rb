  ...
  module Gestion
    class ProductsController < Spree::Admin::BaseController
      ...
      def index
        ...
        respond_with( @concerned_taxons) do |format|
          format.html
      ...
      def json_data
          collection = Spree::Product.where(["name #{LIKE} ?", "%#{params[:q]}%"]).limit(params[:limit] || 10)
          collection.map {|p| {'id' => p.id, 'name' => p.name, 'master' => p.master,
          'variants' => p.variants.map {|v| {"id" => v.id, "reduction"=>v.reduction,
          'option_values' => v.option_values.map {|ov| {"presentation" => ov.presentation,
          'option_type_presentation' => ov.option_type.presentation}}
          }}
          }}.to_json
      end
